...
import dev.jess.baseband.client.api.Utils.BlockUtil;
...
public class NoteBot extends Module {
	public static IRegister[] registers;
	...
	private final File file = new File(Wrapper.mc.gameDir + File.separator + "BaseBand" + File.separator + "notebot");
	private final Setting tune = new Setting("Tune", this, false);
	private final Setting active = new Setting("Active", this, false);
	private final Setting downloadSongs = new Setting("DownloadSongs", this, false);
	...
	public NoteBot() {
		super("NoteBot", "description", Category.WORLD, new Color(129, 163, 199, 255).getRGB());
		BaseBand.settingsManager.rSetting(tune);
		BaseBand.settingsManager.rSetting(active);
		BaseBand.settingsManager.rSetting(downloadSongs);
	...
	public static IRegister[] createRegister(File file, String name) throws IOException {
		int n2;
	...
	@Override
	public boolean onPacketReceive(Packet<?> packet4) {
		if (this.tune.getValBoolean() && packet4 instanceof SPacketBlockAction && this.tuneStage == 0 && this.soundPositions != null) {
			SPacketBlockAction packet = (SPacketBlockAction) packet4;
	...
	public void updatePlayerMotion(int stage) {
		try {
			if (this.downloadSongs.getValBoolean()) {
				this.downloadSongs();
				ChatUtil.sendChatMsg("Songs downloaded");
				this.downloadSongs.setValBoolean(false);
			}
			if (stage == 0) {
				if (this.tune.getValBoolean()) {
					this.tunePre();
				} else if (this.active.getValBoolean()) {
					this.noteBotPre();
				}
			} else if (this.tune.getValBoolean()) {
				this.tunePost();
			} else if (this.active.getValBoolean()) {
				this.noteBotPost();
			}
		} catch (Exception e) {
			ChatUtil.sendChatMsg("Exception in NoteBlock");
			tune.setValBoolean(false);
			active.setValBoolean(false);
		}
	}
	...
	private void tunePre() {
		...
		if (this.tuneStage == 1 && this.getAtomicBlockPos(null) == null) {
			if (this.tuned) {
				ChatUtil.sendChatMsg("Done tuning.");
				this.tune.setValBoolean(false);
			} else {
				this.tuned = true;
				this.tuneStage = 0;
				this.tuneIndex = 0;
	...
	private void noteBotPre() {
		...
		if (registers == null) {
			if (debug.getValBoolean())
				ChatUtil.sendChatMsg("Register Null!");
			return;
		}
		while (this.index < registers.length) {
			IRegister register = registers[this.index];
