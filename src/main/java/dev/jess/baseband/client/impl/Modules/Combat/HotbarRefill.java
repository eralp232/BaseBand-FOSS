...
import dev.jess.baseband.client.api.Utils.Pair;
...
public class HotbarRefill extends Module {
	...
	private final Setting threshold = new Setting("Threshold", this, 1, 32, 62, true);
	private final Setting tickDelay = new Setting("TickDelay", this, 0, 0, 10, true);
	private int delayStep = 0;


	public HotbarRefill() {
		super("HotbarRefill", "From FurryWare, With Love.", Category.COMBAT, new Color(71, 71, 119, 255).getRGB());
		BaseBand.settingsManager.rSetting(threshold);
	...
	private Pair<Integer, Integer> findReplenishableHotbarSlot() {
		...
		for (Map.Entry<Integer, ItemStack> hotbarSlot : getHotbar().entrySet()) {
			...
			if (stack.stackSize > threshold.getValDouble()) {
				continue;
			}
