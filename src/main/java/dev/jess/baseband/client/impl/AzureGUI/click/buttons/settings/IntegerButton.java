...
public class IntegerButton extends Component {
	...
	public IntegerButton(final Setting setting, final ModuleButton parent, final int offset) {
	...
	public void updateComponent(final int mouseX, final int mouseY) {
		...
		renderWidth = 88.0f * ((int) setting.getValDouble() - min) / (max - min);
		if (dragging) {
			if (diff == 0.0) {
				setting.setValDouble(setting.getMin());
			} else {
				final int newValue = (int) roundToPlace(diff / 100.0 * (max - min) + min);
